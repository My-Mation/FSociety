...
def identify_machines_windowed(frames, vibration_percent=None):
    """
    Windowed multi-band matching: detect if ≥2 bands are hit in any frame in batch.
    Fuse vibration: if vibration_percent > 70% and ≥1 band matches, count as detection.
    Args:
        frames: List of frame dicts (each with 'peaks', 'amplitude', etc.)
        vibration_percent: float or None
    Returns:
        Set of detected machine_ids (raw)
    """
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT machine_id, freq_bands, iqr_low, iqr_high FROM machine_profiles ORDER BY machine_id")
        profiles = cursor.fetchall()
        if not profiles:
            return set()

        # For each machine, count band hits across all frames
        band_hits = {mid: 0 for mid, _, _, _ in profiles}
        iqr_hits = {mid: 0 for mid, _, _, _ in profiles}

        for frame in frames:
            peaks_list = frame.get('peaks', [])
            if not peaks_list:
                continue
                    ...
                    for band in freq_bands:
                        ...
                        for peak in peaks_list:
                            ...
                            if band_low <= freq <= band_high:
                                band_match_count += 1
                                break
                    if band_match_count >= MIN_BAND_MATCHES:
                        band_hits[machine_id] += 1
                    elif band_match_count >= 1 and vibration_percent is not None and vibration_percent > 70:
                        # Fuse vibration: if vibration > 70% and ≥1 band matches
                        band_hits[machine_id] += 1
                # Legacy IQR matching
                    ...
                    for peak in peaks_list:
                        ...
                        if iqr_low <= freq <= iqr_high:
                            ...
                            break
                    if iqr_match:
                        iqr_hits[machine_id] += 1

        detected = set()
        # If any frame in batch hits ≥2 bands, count as detected
        for machine_id in band_hits:
            if band_hits[machine_id] > 0:
                detected.add(machine_id)
        for machine_id in iqr_hits:
            if iqr_hits[machine_id] > 0:
                detected.add(machine_id)
        return detected
    finally:
        cursor.close()
            ...
            if valid_raw_values:
                avg_gas = sum(valid_raw_values) / len(valid_raw_values)
                    ...
                    return jsonify({
                        ...
                        "frames_captured": frames_captured,
                        "frames_inserted": inserted_count,
                        "running_machines": sorted(stable_machines),  # Stable (main output)
                        "running_machines_raw": sorted(running_machines_raw),  # Raw windowed detection
                        "all_machines": all_machines
                    })
