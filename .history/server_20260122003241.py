...
MIN_PEAK_AMP = 0.08  # Lower threshold for live FFT
...
def identify_machines(peaks_list):
    ...
    """
    Robust identity + health detection for machine sound.
    Returns:
        {
          machine_id: {
    ...
    try:
        cursor.execute(
            "SELECT machine_id, freq_bands FROM machine_profiles ORDER BY machine_id"
        )
        profiles = cursor.fetchall()

        for machine_id, freq_bands in profiles:
            if not freq_bands:
                continue
            ...
            for band in freq_bands:
                ...
                low = band.get("low")
                high = band.get("high")
                if not center or not low or not high:
                    continue

                width = high - low
                healthy_low  = center - 0.5 * width
                healthy_high = center + 0.5 * width
                identity_low  = center - 3.0 * width
                identity_high = center + 3.0 * width
                ...
                for peak in peaks_list:
                    freq = peak.get("freq")
                    amp  = peak.get("amp", 0)
                    if not freq or amp < MIN_PEAK_AMP:
                        continue

                    if healthy_low <= freq <= healthy_high:
                        healthy += 1
                        band_matched = True
                        print(f"[DEBUG] {machine_id}: freq {freq:.2f} in healthy zone {healthy_low:.2f}-{healthy_high:.2f}")
                        break
                    elif identity_low <= freq <= identity_high:
                        degraded += 1
                        band_matched = True
                        print(f"[DEBUG] {machine_id}: freq {freq:.2f} in identity zone {identity_low:.2f}-{identity_high:.2f}")
                        break

                # Only one peak per band
                if band_matched:
                    continue

            matched = healthy + degraded

            # Allow detection if at least one band matches
            if matched >= 1:
                state = "healthy" if degraded == 0 else "degraded"
                results[machine_id] = {
                    "state": state,
