...
STABILITY_WINDOW = 8  # Track last 8 batches (lowered)
STABILITY_THRESHOLD = 0.4  # Require 40% detection rate (lowered)
    ...
    while True:
        ...
        try:
            ...
            if mode == 'calibration':
                ...
                frames = batch.get('frames', [])
                frames_captured = batch.get('frames_captured', len(frames))
                store_all = batch.get('store_all', False)
                vibration_percent = batch.get('vibration_percent') if 'vibration_percent' in batch else None
                if not frames:
                ...
                running_machines_raw = identify_machines_windowed(valid_frames, vibration_percent)
                cursor.execute("SELECT machine_id FROM machine_profiles")
                all_machines = [row[0] for row in cursor.fetchall()]
                update_detection_history(running_machines_raw, all_machines)
                ...
                print(f"   Detected (raw): {sorted(running_machines_raw)}")
...
def ingest():
    ...
    try:
        ...
        if mode == "calibration":
            ...
            # Get vibration percent from batch if present
            vibration_percent = None
            if "vibration_percent" in data:
                vibration_percent = data["vibration_percent"]
            elif "vibration_summary" in data and isinstance(data["vibration_summary"], dict):
                vibration_percent = data["vibration_summary"].get("vibration_percent")

            # Windowed multi-band matching
            running_machines_raw = identify_machines_windowed(valid_frames, vibration_percent)
            ...
            return jsonify({
                ...
                "running_machines_raw": sorted(running_machines_raw),
...
# =========================
# IDENTIFY MACHINES (WINDOWED, MULTI-BAND)
# =========================
def identify_machines_windowed(frames, vibration_percent=None):
    """Identify machines in a windowed manner using multi-band frequency analysis."""
    from collections import defaultdict
    
    if len(frames) == 0:
        return []
    
    # Group frames by machine_id (for windowed analysis)
    windowed_groups = defaultdict(list)
    for frame in frames:
        machine_id = frame.get("machine_id")
        windowed_groups[machine_id].append(frame)
    
    matched_machines = set()
    
    # Analyze each group (window) separately
    for machine_id, group in windowed_groups.items():
        freqs = []
        all_frequencies = []
        
        # Collect frequencies from peaks
        for frame in group:
            peaks = frame.get("peaks", [])
            amplitude = frame.get("amplitude", 0)
            if amplitude < AMPLITUDE_THRESHOLD:
                continue  # Ignore low amplitude frames
        ...
        if len(freqs) == 0:
            continue  # No valid frequencies in this group
        
        # 1. CLUSTERING OF FREQUENCY PEAKS
        # Use IQR to detect and remove outliers first
        ...
        # Filter out outliers
        filtered_freqs = [f for f in freqs if lower_bound <= f <= upper_bound]
        
        if len(filtered_freqs) == 0:
            continue  # All frequencies are outliers
        
        # Re-compute quartiles and IQR on filtered data
        q1 = percentile(filtered_freqs, 0.25)
        q3 = percentile(filtered_freqs, 0.75)
        iqr = q3 - q1
        
        # Define frequency bands based on IQR
        bands = []
        for i in range(3):
            band_low = q1 - (iqr * (i + 1))
            band_high = q3 + (iqr * (i + 1))
            bands.append((band_low, band_high))
        
        # 2. MATCHING TO MACHINE PROFILES
        cursor = conn.cursor()
        
        # Retrieve all machine profiles
        cursor.execute("SELECT machine_id, freq_bands FROM machine_profiles")
        profiles = cursor.fetchall()
        
        for profile_machine_id, freq_bands_json in profiles:
            if profile_machine_id == machine_id:
                continue  # Skip own machine ID
            
            # Decode JSONB column
            freq_bands = []
            try:
                if freq_bands_json:
                    freq_bands = psycopg2.extras.loads_jsonb(freq_bands_json)
            except Exception as e:
                print(f"[ERROR] Failed to decode JSONB for machine {profile_machine_id}: {str(e)}")
                continue  # Skip this profile on error
            
            # Check each band for overlap
            for band in freq_bands:
                band_center = band.get("center")
                band_low = band.get("low")
                band_high = band.get("high")
                
                if band_center is None or band_low is None or band_high is None:
                    continue  # Incomplete band data, skip
                
                # Check if this band overlaps with the detected frequency range
                if any(low <= band_center <= high for low, high in bands):
                    matched_machines.add(profile_machine_id)
                    break  # No need to check other bands for this profile
        
        cursor.close()
    
    # Convert matched machines to sorted list
    return sorted(matched_machines)

# =========================
# PROFILE SAVING (CALIBRATION)
# =========================
@app.route("/save_profile", methods=["POST"])
def save_profile():
    """Save or update a machine profile based on calibration data"""
    cursor = conn.cursor()
    ...
    try:
        ...
        # 1. COLLECT AND CLUSTER FREQUENCY DATA
        cursor.execute(
            """
        ...
        # 2. CLUSTERING OF FREQUENCY PEAKS
        # Use IQR to detect and remove outliers first
        q1 = percentile(freqs, 0.25)
        q3 = percentile(freqs, 0.75)
        iqr = q3 - q1
        ...
        if len(filtered_freqs) == 0:
            return jsonify({"error": "no_valid_frequencies_after_filtering"}), 400
        ...
        for i in range(3):
            ...
            freq_bands.append({
                ...
                "samples": len([f for f in filtered_freqs if band_low <= f <= band_high])
            })
