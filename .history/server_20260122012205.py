    ...
    try:
        ...
        if mode == "calibration":
            ...
            return jsonify({
                ...
                if len(freq_bands) < 1:
                    return jsonify({"error": "Could not identify any frequency bands from calibration data"}), 400
                ...
                cursor.execute(
                    ...
                    (machine_id, median_freq, iqr_low, iqr_high,
                     ...
                     psycopg2.extras.Json(vibration_data) if 'vibration_data' in locals() and vibration_data else None,
                     psycopg2.extras.Json(gas_data) if 'gas_data' in locals() and gas_data else None)
                )
                conn.commit()
                print(f"   Vibration data: {vibration_data if 'vibration_data' in locals() else None}")
                print(f"   Gas data: {gas_data if 'gas_data' in locals() else None}")
                return jsonify({
                    ...
                    "frames_used": n_total,
                    "vibration_data": vibration_data if 'vibration_data' in locals() else None,
                    "gas_data": gas_data if 'gas_data' in locals() else None
                })
                    valid_frames.append(frame)
                conn.commit()
                vibration_percent = None
