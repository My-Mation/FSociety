...
def ensure_db_schema():
    ...
    try:
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_esp32_device_id ON esp32_data(device_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_esp32_timestamp ON esp32_data(timestamp);")
...
def ingest_esp32():
    """Ingest sensor data from ESP32 devices (vibration, gas sensors)"""
    cursor = conn.cursor()
    try:
        data = request.get_json(force=True)
        ...
        conn.commit()
        print(f"[OK] ESP32 DATA: device={device_id}, vib={vibration}, gas={gas_raw}")
        return jsonify({"status": "ok"})

    except Exception as e:
        conn.rollback()
        print("[ERROR] ESP32 INGEST:", str(e))
        traceback.print_exc()
        return jsonify({"error": "server error"}), 500
    finally:
        cursor.close()
