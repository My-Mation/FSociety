...
# --- Winner-takes-all temporal lock state ---
HEALTHY_RANGE = 50
ANOMALY_RANGE = 150
MIN_PEAK_AMP = 0.05
        ...
        try:
            ...
            if mode == 'calibration':
                ...
                for frame in frames:
                    ...
                    detections = identify_machines(peaks)
                    for mid, info in detections.items():
                        running_machines.add(mid)
                        if info["state"] == "degraded":
                            print(f"[WARN] {mid} detected but unhealthy")
...
# --- Winner-takes-all single-band detector with temporal lock ---
def identify_machine(peaks):
    global CURRENT_MACHINE, CURRENT_STATE, LOCK_UNTIL

    if not peaks or not isinstance(peaks, list) or len(peaks) == 0:
        return None

    # Filter out low-frequency noise and weak peaks
    valid_peaks = [p for p in peaks if p.get("freq", 0) >= 60 and p.get("amp", 0) >= MIN_PEAK_AMP]
    if not valid_peaks:
        return None

    now = time.time()

    # If locked, keep returning the same machine
    if CURRENT_MACHINE and now < LOCK_UNTIL:
        return {
            "machine": CURRENT_MACHINE,
            "state": CURRENT_STATE
        }

    dominant_freq = valid_peaks[0].get("freq")
    amp = valid_peaks[0].get("amp", 0)

    if not dominant_freq or amp < MIN_PEAK_AMP:
        return None

    cursor = conn.cursor()
    try:
        cursor.execute(
            "SELECT machine_id, median_freq FROM machine_profiles"
        )
        ...
        for machine_id, median in machines:
            if not median:
                continue

            # Healthy window
            if abs(dominant_freq - median) <= HEALTHY_RANGE:
                score = 2
                state = "healthy"

            # Anomaly window
            elif abs(dominant_freq - median) <= ANOMALY_RANGE:
                score = 1
                state = "degraded"
        ...
        def ingest():
            ...
            try:
                ...
                # ---- LIVE DETECTION MODE: Process frames and detect ----
                    ...
                    for frame in frames:
                        peaks = frame.get("peaks", [])
                        result = identify_machine(peaks)
                        if result:
                            detected_machine = result["machine"]
                            detected_state = result["state"]
                            break  # Only one machine can be detected at a time
                        # ...existing code for inserting frame if needed...
                        pass

                    conn.commit()

                    print(f"\n[OK] LIVE BATCH: {len(frames)} frames, {inserted_count} inserted (store_all={store_all})")
                    print(f"   Detected machine: {detected_machine} (state: {detected_state})")
...
def live_status():
    ...
    try:
        ...
        # Optionally, re-run identify_machines on last batch peaks to get degraded states
        # This requires storing last batch peaks, but for now, we can infer degraded from logs or extend detection_history
        # For demonstration, we will not re-query peaks, but you can extend detection_history to store state if needed

        return jsonify({
            "detected": sorted(detected),
            "stable": sorted(stable),
            "degraded": sorted(degraded)  # Placeholder, can be populated if state tracking is extended
        })

    except Exception as e:
        print(f"[ERROR] LIVE_STATUS ERROR: {str(e)}")
        return jsonify({"detected": [], "stable": [], "degraded": []})
