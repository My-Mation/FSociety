...
def get_session_preview():
    ...
    print(f"[DEBUG] SESSION_PREVIEW called with start={start_ts}, stop={stop_ts}, machine_id={machine_id}, device_id={device_id}")

    if not start_ts or not stop_ts:
        print("[DEBUG] Missing start or stop timestamp")
        return jsonify({"error": "start and stop timestamps are required"}), 400

    try:
        print(f"[DEBUG] Calling aggregate_session_data with conn={conn is not None}")
